...
public class Service {
    ...
    public Response isStoreKeeper(String userName){
        return userService.isStoreKeeper(userName);
    }
    ...
    public Response showBranchesID(String userName){
        Response r = userService.HR_Or_KickOut(userName);
        Response rs = userService.TM_Or_KickOut(userName);
        if (!r.isError || !rs.isError) {
    ...
    public Response createNewRole(String userName, String role){
        Response r = userService.HR_Or_KickOut(userName);
        if (!r.isError) {
            r = shiftService.createNewRole(role);
        }
        return r;
    }
    ...
    public Response getMissingRequirementsForShift(String BID, String shiftSign){
        return shiftService.getMissingRequirementsForShift(BID, shiftSign);
    }

    public Response pickEmployee(String userName, String BID, String dayAndType, String role, String employeeName, String TID){
        Response r = userService.HR_Or_KickOut(userName);
        if (!r.isError) {
            r = shiftService.pickEmployee(BID, dayAndType, role, employeeName, TID);
        }
        return r;
    }
    ...
    // --- TRANSPORTS --- //
    public Response showUnassignedTransports(String userName, String BID, String shiftSign){
        Response r = userService.HR_Or_KickOut(userName);
        if (!r.isError) {
            r = shiftService.showUnassignedTransports(BID, shiftSign);
        }
        return r;
    }

    public Response addTransport(String userName, String BID, String shiftSign, String Weight){
        Response r = userService.TM_Or_KickOut(userName);
        if (!r.isError) {
            r = shiftService.addTransport(BID, shiftSign, Weight);
        }
        return r;
    }

    public Response showTransportHistory(String userName){
        Response r = userService.TM_Or_KickOut(userName);
        if (!r.isError) {
            r = shiftService.showTransportHistory();
        }
        return r;
    }

    public Response showTransportHistoryByBranch(String userName, String BID){
        Response r = userService.TM_Or_KickOut(userName);
        if (!r.isError) {
            r = shiftService.showTransportHistoryByBranch(BID);
        }
        return r;
    }
